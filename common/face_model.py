...
class FaceModel:
    ...
    人脸模型正对着相机，z轴垂直于由眼睛中心和嘴巴中心形成的平面，x轴平行于两个眼睛中心连接的直线，原点位于鼻尖
    ...
    def estimate_head_pose(self, face: Face, camera: Camera) -> None:
        ...
        # 返回的rvec和tvec是检测到的人脸坐标系到相机坐标系的旋转向量和平移向量
        _, rvec, tvec = cv2.solvePnP(self.LANDMARKS,        # 通用人脸模型关键点
                                     face.landmarks,        # 检测到的人脸关键点
                                     camera.camera_matrix,  # 相机内参矩阵
                                     camera.dist_coefficients,  # 相机畸变系数
                                     rvec,      # 输出的旋转向量。使坐标点从通用人脸模型坐标系旋转到相机坐标系
                                     tvec,      # 输出的平移向量。使坐标点从通用人脸模型坐标系平移到相机坐标系
